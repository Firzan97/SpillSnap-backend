...
export class StatCounterDto {
  ...
  @ApiProperty({
    example: 18400,
    description:
      "Recent gain over the stat's natural window (today / week / month)",
  })
  delta: number;
}
...
export class TopSnapperDto {
  ...
  @ApiProperty({
    example: 'Priya R.',
    description: 'Anonymized display handle',
  })
  name: string;
...
export class PublicStatsResponseDto {
  ...
  @ApiProperty({
    type: StatCounterDto,
    description:
      'SST captured on tax-eligible receipts (RM), used as the public "tax savings" proxy',
  })
  taxSavings: StatCounterDto;

  @ApiProperty({
    type: [TopSnapperDto],
    description: 'Anonymized top snappers this month',
  })
